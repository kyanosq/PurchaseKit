...
1. **当前已验证权益**：来自 `Transaction.currentEntitlements`（已验证、未撤销，含账单宽限期），
   决定当前在线授权。已验证的撤销交易不授予任何权益，也不进入历史 / 活跃快照。
2. **历史购买身份**：来自经过验证后写入的本地订阅历史，与当前权益 ID 分离、仅累加，
...
| 字段 | 来源 |
| --- | --- |
| `hasActiveSubscription` | 当前已验证的自动续订交易（含账单宽限） |
| `hasLifetime` | 已验证终身交易或启动时从缓存还原的终身权益；完整快照缺失/撤销时清除 |
...
- 终身买断缓存不随订阅离线宽限过期；启动时可还原 `.lifetime`。已验证撤销或完整快照中缺失会清除该证据，不永久覆盖 StoreKit 当前状态。
