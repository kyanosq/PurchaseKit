...
xcodebuild -scheme PurchaseKit \
  ...
  -only-testing:PurchaseKitTests/EntitlementVerificationTests \
...
> 这条路径在 0.2.0 之前是不存在的：`Package.swift` 只声明 `.iOS(.v17)`，其余平台回落到
> SwiftPM 的默认部署目标（macOS 10.13），`os.Logger` 无法编译，`swift build` 直接失败。
> 后果不是「macOS 上不可用」，而是整套单元测试**从未进过任何命令行门禁**。
